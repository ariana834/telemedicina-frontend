...
function SymptomStep({ consultation, onSymptomAdded, onEmergency }) {
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            if (updated.status === 'EMERGENCY_REDIRECT') {
                onEmergency(updated);
                return;
            }
            if (updated.status === 'FORM_GENERATED' || newList.length === 3) {
                onSymptomAdded(updated);
            }
        } catch (e) {
            setError(e.message);
        } finally {
                    ...
                    {[0, 1, 2].map((i) => {
                        ...
                        return (
                            <Box key={i} sx={{
                                ...
                                bgcolor: s ? 'rgba(139,115,85,0.05)' : 'transparent',
                                transition: 'all 0.2s',
                            }}>
                                <Box sx={{
                                    width: 28, height: 28, borderRadius: '50%',
                                    bgcolor: s ? '#8B7355' : '#EDE5D8',
                                    display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
                                }}>
                                    {s
                                        ? <CheckCircleIcon sx={{ fontSize: '1rem', color: '#FFFCF8' }} />
                                ...
                                {s ? (
                                    <Box sx={{ flex: 1 }}>
                                        ...
                                        <Chip label={s.severity} size="small" sx={{
                                            mt: 0.3, fontSize: '0.7rem', fontFamily: 'Lato', fontWeight: 700,
                ...
                {count < 3 && (
                    <Card elevation={0} sx={{ border: '1.5px solid #EDE5D8', borderRadius: 2, p: 2.5, bgcolor: '#FFFCF8' }}>
                        <Typography sx={{ fontFamily: 'Lato', fontWeight: 700, color: '#5C4A32', fontSize: '0.85rem', mb: 1.5, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                            Symptom {count + 1} of 3
                        </Typography>
                        <TextField
                            fullWidth label="Describe symptom"
                            placeholder="e.g. headache, fever, abdominal pain..."
                            value={symptomText}
                            onChange={(e) => setSymptomText(e.target.value)}
                            onKeyDown={(e) => e.key === 'Enter' && canAdd && handleAdd()}
                            disabled={loading} size="small" sx={{ ...inputSx, mb: 2 }}
                        />
...
function MedicalFormStep({ consultationId, onFormSubmitted }) {
        ...
        try {
            ...
            const payload = questions
                .map((q) => {
                    ...
                    const answerText = q.questionType === 'CHECKBOX'
                        ? (Array.isArray(raw) ? raw.join(', ') : raw || '')
                        : (raw || '');
                    return { questionId: q.id, answerText };
                })
                .filter((a) => {
                    if (!a.answerText || a.questionId == null) return false;
    ...
    const answered = questions.filter((q) => {
        const ans = answers[q.id];
        return q.questionType === 'CHECKBOX' ? ans?.length > 0 : ans?.trim();
    }).length;
    ...
    return (
        <Fade in timeout={400}>
            <Box>
                ...
                <Typography sx={{ fontFamily: 'Lato', color: '#9E8B72', fontSize: '0.88rem', mb: 0.5 }}>
                    This form was generated based on your symptoms and medical history. Please answer carefully.
                </Typography>
                ...
                <Box sx={{ mb: 3 }}>
                    ...
                    <LinearProgress variant="determinate" value={(answered / Math.max(questions.length, 1)) * 100}
                                    sx={{ height: 5, borderRadius: 3, bgcolor: '#EDE5D8', '& .MuiLinearProgress-bar': { bgcolor: '#8B7355', borderRadius: 3 } }} />
                </Box>
                        ...
                        <Card key={q.id} elevation={0} sx={{ border: '1.5px solid #EDE5D8', borderRadius: 2, bgcolor: '#FFFCF8', p: 2.5 }}>
                            ...
                            {q.questionType === 'YES_NO' && (
                                <Box sx={{ display: 'flex', gap: 1.5 }}>
                                    {['Yes', 'No'].map((opt) => (
                                        <Box key={opt} onClick={() => setAnswer(q.id, opt)} sx={{
                                            flex: 1, textAlign: 'center', py: 1.2, borderRadius: 1.5,
                                            border: answers[q.id] === opt ? '2px solid #8B7355' : '1.5px solid #EDE5D8',
                                            bgcolor: answers[q.id] === opt ? 'rgba(139,115,85,0.08)' : 'transparent',
                                            cursor: 'pointer', transition: 'all 0.15s', '&:hover': { borderColor: '#8B7355' },
                                        }}>
                            ...
                            {q.questionType === 'MULTIPLE_CHOICE' && (
                                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                                    {(q.options || []).map((opt) => (
                                        <Chip key={opt} label={opt} onClick={() => setAnswer(q.id, opt)} sx={{
                                            fontFamily: 'Lato', fontSize: '0.82rem', cursor: 'pointer',
                                            border: answers[q.id] === opt ? '1.5px solid #8B7355' : '1.5px solid #EDE5D8',
                                            bgcolor: answers[q.id] === opt ? 'rgba(139,115,85,0.1)' : 'transparent',
                                            color: answers[q.id] === opt ? '#8B7355' : '#6B5A47',
                                            fontWeight: answers[q.id] === opt ? 700 : 400,
                                            '&:hover': { bgcolor: 'rgba(139,115,85,0.07)' },
                                        }} />
                                    ))}
                                </Box>
                            )}
                            ...
                            {q.questionType === 'CHECKBOX' && (
                                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                                    {(q.options || []).map((opt) => {
                                        ...
                                        return (
                                            <Chip key={opt} label={opt} onClick={() => toggleCheckbox(q.id, opt)}
                                                  icon={selected ? <CheckCircleIcon sx={{ fontSize: '0.9rem !important', color: '#8B7355 !important' }} /> : undefined}
                                                  sx={{
                                                      fontFamily: 'Lato', fontSize: '0.82rem', cursor: 'pointer',
                                                      border: selected ? '1.5px solid #8B7355' : '1.5px solid #EDE5D8',
                                                      bgcolor: selected ? 'rgba(139,115,85,0.1)' : 'transparent',
                                                      color: selected ? '#8B7355' : '#6B5A47',
                            ...
                            {q.questionType === 'OPEN_TEXT' && (
                                <TextField fullWidth multiline rows={2} placeholder="Your answer..."
                ...
                <Button fullWidth variant="contained" disabled={!isComplete() || submitting} onClick={handleSubmit}
                    ...
                    {submitting ? (
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            <CircularProgress size={18} sx={{ color: '#FFFCF8' }} />
                            <span>Computing diagnosis...</span>
                        </Box>
...
function DiagnosisStep({ consultationDetail, consultationId, onDone }) {
    ...
    const handleSchedule = async () => {
        ...
        try {
            const res = await scheduleAppointment(token, consultationId);
            setResult({ type: 'appointment', id: res.appointmentId });
    ...
    const handlePrescribe = async () => {
        setLoading(true); setError('');
        try {
            const res = await prescribe(token, consultationId);
            setResult({ type: 'prescription', id: res.prescriptionId });
        } catch (e) { setError(e.message); } finally { setLoading(false); }
    };
                ...
                <Box sx={{ textAlign: 'center' }}>
                    ...
                    <Typography sx={{ fontFamily: 'Lato', color: '#9E8B72', fontSize: '0.9rem', mb: 3 }}>
                        {result.type === 'prescription'
                            ? `Prescription #${result.id} generated with OTC medications. View it in your Prescriptions section.`
                            : `Appointment #${result.id} scheduled automatically. Check your Appointments section for details.`}
                    </Typography>
                    <Stack direction="row" spacing={1.5} justifyContent="center">
                        <Button variant="outlined" onClick={() => onDone(result.type === 'prescription' ? '/prescriptions' : '/appointments')}
                                sx={{ fontFamily: 'Lato', fontWeight: 700, borderColor: '#8B7355', color: '#8B7355', borderRadius: 2, '&:hover': { bgcolor: 'rgba(139,115,85,0.07)' } }}>
                            View {result.type === 'prescription' ? 'Prescription' : 'Appointment'}
                        </Button>
                        <Button variant="contained" onClick={() => onDone('/dashboard')}
                                sx={{ fontFamily: 'Lato', fontWeight: 700, bgcolor: '#8B7355', borderRadius: 2, '&:hover': { bgcolor: '#7A6348' } }}>
                            Back to Dashboard
                        </Button>
                    </Stack>
                </Box>
    ...
    return (
        <Fade in timeout={400}>
            <Box>
                ...
                <Typography sx={{ fontFamily: 'Lato', color: '#9E8B72', fontSize: '0.88rem', mb: 3 }}>
                    Based on your symptoms and form responses, our system generated the following preliminary assessment.
                </Typography>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 2, borderRadius: 2, mb: 3, bgcolor: meta.bg, border: `1.5px solid ${meta.color}33` }}>
                    ...
                    <Box>
                        <Typography sx={{ fontFamily: 'Lato', fontWeight: 700, color: meta.color, fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: 0.5 }}>
                            Case Complexity — {meta.label}
                        </Typography>
                    ...
                    {[primary, secondary].filter(Boolean).map((d, i) => (
                        <Card key={d.id} elevation={0} sx={{ border: '1.5px solid #EDE5D8', borderRadius: 2, bgcolor: '#FFFCF8', p: 2.5 }}>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1 }}>
                                <Box sx={{ flex: 1 }}>
                                    <Typography sx={{ fontFamily: 'Lato', fontSize: '0.72rem', color: '#9E8B72', textTransform: 'uppercase', letterSpacing: 0.5, mb: 0.3 }}>
                                        {i === 0 ? 'Primary diagnosis' : 'Secondary diagnosis'}
                                    </Typography>
                ...
                {complexity === 'EMERGENCY' && (
                    <Box sx={{ bgcolor: 'rgba(244,67,54,0.06)', border: '2px solid rgba(244,67,54,0.3)', borderRadius: 2, p: 3, textAlign: 'center' }}>
                        ...
                        <Typography sx={{ fontFamily: 'Lato', fontSize: '0.88rem', color: '#6B5A47', mb: 2 }}>Your symptoms indicate a potentially serious condition requiring immediate in-person evaluation. Go to the nearest UPU now.</Typography>
                ...
                {complexity === 'SIMPLE' && (
                    <Card elevation={0} sx={{ border: '1.5px solid #EDE5D8', borderRadius: 2, bgcolor: '#FFFCF8', p: 2.5 }}>
                        ...
                        <Button fullWidth variant="contained" disabled={loading} onClick={handlePrescribe}
                                startIcon={loading ? <CircularProgress size={16} sx={{ color: 'inherit' }} /> : <MedicalServicesIcon />}
                ...
                {(complexity === 'MEDIUM' || complexity === 'COMPLEX') && (
                    <Card elevation={0} sx={{ border: '1.5px solid #EDE5D8', borderRadius: 2, bgcolor: '#FFFCF8', p: 2.5 }}>
                        ...
                        <Button fullWidth variant="contained" disabled={loading} onClick={handleSchedule}
...
export default function NewConsultationPage() {
    ...
    const handleStart = async () => {
        ...
        try {
            const c = await createConsultation(token);
            setConsultation(c);
            setStarted(true);
    ...
    return (
        <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: '#F7F3EE' }}>
            <Sidebar />
            <Box component="main" sx={{ flexGrow: 1, px: { xs: 2.5, md: 5 }, py: 4.5, maxWidth: 760, mx: 'auto', width: '100%' }}>
                    ...
                    <Card elevation={0} sx={{ border: '1.5px solid #EDE5D8', borderRadius: 3, bgcolor: '#FFFCF8', p: { xs: 2.5, md: 4 } }}>
                        <Box sx={{ textAlign: 'center', py: 2 }}>
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, mb: 4 }}>
                                {[
                                    { label: 'Add 3 symptoms',       icon: '🩺' },
                                    { label: 'Complete medical form', icon: '📋' },
                                    { label: 'Get diagnosis',         icon: '🔬' },
                                ].map((item, i) => (
                                    <Box key={i} sx={{
                                        width: 130, p: 2.5, borderRadius: 2,
                                        border: '1.5px solid #EDE5D8',
                                        bgcolor: 'rgba(139,115,85,0.03)',
                                        textAlign: 'center',
                                    }}>
                                        <Typography sx={{ fontSize: '1.8rem', mb: 1 }}>{item.icon}</Typography>
                                        <Typography sx={{ fontFamily: 'Lato', fontSize: '0.78rem', color: '#6B5A47', fontWeight: 600, lineHeight: 1.4 }}>
                                            {item.label}
                                        </Typography>
                                    </Box>
                                ))}
                            ...
                            <Collapse in={!!createError}>
                                <Alert severity="error" sx={{ mb: 2, borderRadius: 2, fontFamily: 'Lato', textAlign: 'left' }}>{createError}</Alert>
                            </Collapse>
                            ...
                            <Button variant="contained" size="large" disabled={creating} onClick={handleStart}
                                ...
                                {creating
                                    ? <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><CircularProgress size={18} sx={{ color: '#FFFCF8' }} /><span>Starting...</span></Box>
                                    : 'Start Consultation'}
                            </Button>
                        </Box>
                    </Card>
                    ...
                    <Card elevation={0} sx={{ border: '1.5px solid #EDE5D8', borderRadius: 3, bgcolor: '#FFFCF8', p: { xs: 2.5, md: 4 } }}>
                        <Stepper activeStep={activeStep} sx={{ mb: 4 }}>
                            {STEPS.map((label) => (
                                <Step key={label}>
                                    <StepLabel sx={{
                                        '& .MuiStepLabel-label': { fontFamily: 'Lato', fontSize: '0.82rem', color: '#9E8B72' },
                                        '& .MuiStepLabel-label.Mui-active': { color: '#5C4A32', fontWeight: 700 },
                                        '& .MuiStepLabel-label.Mui-completed': { color: '#8B7355' },
                                        '& .MuiStepIcon-root': { color: '#D4C9B8' },
                        ...
                        {activeStep === 0 && consultation && (
                            <SymptomStep consultation={consultation} onSymptomAdded={handleSymptomsDone} onEmergency={handleEmergency} />
                        )}
                        {activeStep === 1 && consultation && (
                            <MedicalFormStep consultationId={consultation.id} onFormSubmitted={handleFormSubmitted} />
                        )}
                        {activeStep === 2 && consultDetail && (
                            <DiagnosisStep consultationDetail={consultDetail} consultationId={consultation.id} onDone={handleDone} />
                        )}
                    </Card>
                )}
            </Box>
